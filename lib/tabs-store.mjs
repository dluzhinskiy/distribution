...
const TABLES = {
  cases: {
    ...
    keyFields: ["case_id"],
    dateFields: ["Дата поступления", "Дата завершения", "Дата распределения"],
  },
  employees: {
    name: "Сотрудники",
  ...
  settings: {
    ...
    keyFields: ["Тип дела"],
    dateFields: [],
    numberFields: ["Срок актуальности"],
  },
};
...
export async function readData() {
  const [cases, employees, queues, state, vacations, journal, settings] = await Promise.all([
  ...
  return { cases, employees, queues, state, vacations, journal, settings };
}
