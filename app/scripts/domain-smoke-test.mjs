...
{
  const data = baseData({ regionalLoad: 5, regionalEnabled: "Да" });
  data.regionalAssignments.push({
    "ЮЦ": "Дальний Восток",
    "Регион": "Хабаровский край",
  ...
  data.vacations = [{
    ...
    "Дата окончания": "2026-06-28",
    "Тип": "Отпуск",
  }];
  data.state[0]["Последняя позиция"] = 0;
  data.state[0]["Последний автоназначенный"] = "";
  const assigned = assignAutomatically(data, { ...draft(), "Предмет": "Тест долга в региональной очереди" }, date);
  assert.equal(assigned.case["Ответственный"], "Петров П.П.");
  assert.equal(data.queues[0]["Долг"], 1);
}

{
  const data = baseData({ regionalEnabled: "Да" });
  data.employees = [
    { employee_id: "E1", "ФИО": "Иванов И.И.", "ЮЦ": "Дальний Восток", "Активен": "Да", "Судебные": "Да", "Административные": "Да", "Претензии": "Да" },
    { employee_id: "E2", "ФИО": "Петров П.П.", "ЮЦ": "Дальний Восток", "Активен": "Да", "Судебные": "Да", "Административные": "Да", "Претензии": "Да" },
    { employee_id: "E3", "ФИО": "Сидоров С.С.", "ЮЦ": "Дальний Восток", "Активен": "Да", "Судебные": "Да", "Административные": "Да", "Претензии": "Да" },
  ];
  data.queues = [
    { queue_id: "QUEUE-Дальний Восток-судебное", employee_id: "E2", "ФИО": "Петров П.П.", "ЮЦ": "Дальний Восток", "Тип дела": "судебное", "Позиция": 1, "Долг": 0 },
    { queue_id: "QUEUE-Дальний Восток-судебное", employee_id: "E3", "ФИО": "Сидоров С.С.", "ЮЦ": "Дальний Восток", "Тип дела": "судебное", "Позиция": 2, "Долг": 0 },
    { queue_id: "QUEUE-Дальний Восток-судебное", employee_id: "E1", "ФИО": "Иванов И.И.", "ЮЦ": "Дальний Восток", "Тип дела": "судебное", "Позиция": 3, "Долг": 0 },
  ];
  data.state[0]["Последняя позиция"] = 0;
  data.state[0]["Последний автоназначенный"] = "";
  data.settings = [{
  ...
  data.regionalAssignments = [
    { "ЮЦ": "Дальний Восток", "Регион": "Приморский край", "Сотрудник": "Иванов И.И.", "Тип нагрузки": "судебное", "Активно": "Да" },
    { "ЮЦ": "Дальний Восток", "Регион": "Хабаровский край", "Сотрудник": "Иванов И.И.", "Тип нагрузки": "судебное", "Активно": "Да" },
  ];
  data.vacations = [{
    employee_id: "E1",
  ...
  const assigned = assignAutomatically(data, {
    "ЮЦ": "Дальний Восток",
    "Регион": "Приморский край",
    "Тип дела": "судебное",
    "Предмет": "Тест долга при уходе из региональной очереди",
  }, date);
  assert.equal(assigned.case["Ответственный"], "Петров П.П.");
  assert.match(assigned.result.basis, /общая очередь после недоступности региональных юристов/);
  assert.equal(data.queues.find((row) => row.employee_id === "E1")["Долг"], 1);
}

{
  ...
  data.regionalSubstitutions = [
    { "ЮЦ": "Дальний Восток", "Регион": "Приморский край", "Основной сотрудник": "Иванов И.И.", "Замещающий сотрудник": "Сидоров С.С.", "Тип нагрузки": "судебное", "Активно": "Да" },
  ];
  data.vacations = [{
    employee_id: "E1",
    "ФИО": "Иванов И.И.",
  ...
  const assigned = assignAutomatically(data, {
    ...
    "Регион": "Приморский край",
    "Тип дела": "судебное",
    "Предмет": "Тест заместителя на месте регионального сотрудника",
  }, date);
  assert.equal(assigned.case["Ответственный"], "Сидоров С.С.");
  assert.match(assigned.result.basis, /региональная очередь: Приморский край \(замещение\)/);
  assert.equal(assigned.result.position, 3);
  assert.equal(data.state[0]["Последняя позиция"], 3);
  assert.equal(data.queues.find((row) => row.employee_id === "E1")["Долг"], 1);
}
