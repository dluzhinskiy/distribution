...
function caseStatusSelect(row) {
  return `<select class="inline-select case-status" data-id="${escapeHtml(row.case_id)}">
    ${statuses.map((status) => `<option value="${status}" ${row["Статус"] === status ? "selected" : ""}>${status}</option>`).join("")}
  </select>`;
}
...
function renderCases() {
  ...
  $("#casesTable").innerHTML = rows.map((row) => `
    <tr>
      <td class="id-cell">${escapeHtml(row.case_id)}</td>
...
function bindEvents() {
  ...
  document.addEventListener("change", (event) => {
    if (event.target.matches(".case-status")) {
      saveCaseStatus(event.target.dataset.id, event.target.value).catch((error) => toast(error.message, "error"));
    }
