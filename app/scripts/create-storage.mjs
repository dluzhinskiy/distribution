...
async function loadWorkbook() {
  const source = (await exists(MVP_SOURCE)) ? MVP_SOURCE : ORIGINAL_SOURCE;
  return {
    workbook: XLSX.readFile(decodeURIComponent(source.pathname), { cellDates: true }),
    source: decodeURIComponent(source.pathname).replace(decodeURIComponent(ROOT.pathname), ""),
  };
}
