  ...
  if (req.method === "PATCH" && url.pathname.startsWith("/api/yuc-settings/")) {
    ...
    if (!row) {
      row = {
        "Название": "",
        [FIELD.yuc]: yuc,
        [YUC_SETTING.regionalEnabled]: "Да",
    ...
    const confirmedData = await confirmedDataAfterSave(data, (freshData) => Boolean(freshData.yucSettings.find((item) => normalizeYuc(item[FIELD.yuc]) === yuc)));
