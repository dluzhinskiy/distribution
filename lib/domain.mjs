...
export function enrichData(data, date = new Date()) {
  ...
  const employees = data.employees.map((employee) => {
    ...
    const activeCases = activeRegisterCases.filter((caseRow) => nameMatches(caseRow["Ответственный"], name) && caseRow["Активное число"] === 1);
    return {
      ...employee,
  ...
  const summary = {
    ...
    unassignedCases: activeRegisterCases.filter((item) => !cleanText(item["Ответственный"])).length,
    activeEmployees: employees.filter((item) => yes(item["Активен"])).length,
    byType: CASE_TYPES.map((type) => ({
      type,
      total: activeRegisterCases.filter((item) => item["Тип дела"] === type).length,
      active: activeRegisterCases.filter((item) => item["Тип дела"] === type && item["Активное число"] === 1).length,
...
function employeeForQueueRow(data, queueRow) {
  return employeeById(data, queueRow.employee_id) ?? employeeByName(data, queueRow["ФИО"]);
}
