...
function summaryForSelectedYuc() {
  ...
  const byTypeSource = state.data?.summary?.byType ?? [];
  return {
    ...
    byType: caseTypes.map((type) => {
      ...
      return {
        ...
        relevanceDays: byTypeSource.find((item) => item.type === type)?.relevanceDays ?? "—",
      };
    }),
  };
}
...
function renderManualSelect() {
  ...
  select.innerHTML = `<option value="">Не использовать</option>` + employees
...
function renderEmployees() {
  ...
  `).join("") : `<tr><td colspan="8" class="empty-cell">Сотрудники выбранного ЮЦ не найдены.</td></tr>`;
}
...
function renderVacationCalendar() {
  ...
  $("#vacationWarning").textContent = state.vacationEdit
    ? "Редактирование включено. Клики по календарю сразу сохраняются в MTS Tabs."
    : "Редактирование выключено. Чтобы менять отпуск, включите тумблер — тогда клики по календарю будут сразу сохраняться в MTS Tabs.";
}
...
function renderQueues() {
  const queues = queuesForSelectedYuc();
  $("#queuesTable").innerHTML = queues.length ? queues.map((row) => `
    <tr>
      <td>${escapeHtml(row["Тип дела"])}</td>
      <td>${escapeHtml(row["Позиция"])}</td>
      <td title="${escapeHtml(row["ФИО"])}">${escapeHtml(displayName(row["ФИО"]))}</td>
      <td>
        <select class="inline-select queue-field" data-queue="${escapeHtml(row.queue_id)}" data-employee="${escapeHtml(row.employee_id)}" data-field="Долг">
          <option value="0" ${Number(row["Долг"]) !== 1 ? "selected" : ""}>Нет</option>
          <option value="1" ${Number(row["Долг"]) === 1 ? "selected" : ""}>Да</option>
        </select>
      </td>
  ...
  `).join("") : `<tr><td colspan="5" class="empty-cell">Очереди выбранного ЮЦ не найдены.</td></tr>`;

  const stateRows = stateRowsForSelectedYuc();
  $("#stateTable").innerHTML = stateRows.length ? stateRows.map((row) => `
    <tr>
      <td>${escapeHtml(row["Тип дела"])}</td>
      <td>${escapeHtml(row["Последняя позиция"])}</td>
      <td>${escapeHtml(row["Последний автоназначенный"] || "—")}</td>
  ...
  `).join("") : `<tr><td colspan="4" class="empty-cell">Состояние очередей выбранного ЮЦ не найдено.</td></tr>`;
}
...
async function clearVacationYear() {
  ...
  const year = selectedVacationYear();
  if (!employee) return;
  const confirmed = window.confirm(`Снять все отпуска ${displayName(employee["ФИО"])} за ${year} год?`);
  if (!confirmed) return;
