...
async function api(req, res, url) {
  ...
  if (req.method === "PATCH" && url.pathname.startsWith("/api/yuc-settings/")) {
    ...
    const patch = await readBody(req);
    const overloadThreshold = Number(patch[YUC_SETTING.overloadThreshold]);
    if (!Number.isFinite(overloadThreshold) || overloadThreshold < 0) {
      throw new Error("Порог перегруза должен быть неотрицательным числом.");
    }
    const overloadMode = cleanText(patch[YUC_SETTING.overloadMode]) || "общая нагрузка";
    if (!["общая нагрузка", "тип нагрузки"].includes(overloadMode)) {
      throw new Error("Недопустимый режим расчёта перегруза.");
    }
    const data = await readData();
    ...
    Object.assign(row, {
      ...
      [YUC_SETTING.overloadThreshold]: overloadThreshold,
      [YUC_SETTING.overloadMode]: overloadMode,
    ...
    const confirmedData = await confirmedDataAfterSave(data, ["yucSettings"], (freshData) => {
      const saved = freshData.yucSettings.find((item) => normalizeYuc(item[FIELD.yuc]) === yuc);
      return cleanText(saved?.[YUC_SETTING.overloadMode]) === overloadMode;
    });
    const savedSettings = confirmedData.yucSettings.find((item) => normalizeYuc(item[FIELD.yuc]) === yuc);
    if (cleanText(savedSettings?.[YUC_SETTING.overloadMode]) !== overloadMode) {
      throw new Error(`Результат операции требует проверки: MTS Tabs не подтвердил значение «${overloadMode}» для настройки «${YUC_SETTING.overloadMode}».`);
    }
    return sendJson(res, 200, { ok: true, settings: savedSettings, data: confirmedData });
  }
