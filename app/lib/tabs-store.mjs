...
async function syncTable(key, desiredRows = []) {
  ...
  if (table.appendOnly) {
    ...
    for (const row of desiredRows) {
      if (row._recordId && currentByRecordId.has(row._recordId)) {
...
export async function readData() {
  const [
    ...
    readTable(TABLES.employees),
    readTable(TABLES.queues),
    readTable(TABLES.state),
    readTable(TABLES.vacations),
    readTable(TABLES.journal),
    readTable(TABLES.settings),
    readTable(TABLES.yucSettings),
    readTable(TABLES.regionalAssignments),
    readTable(TABLES.regionalSubstitutions),
  ]);
  return { cases, employees, queues, state, vacations, journal, settings, yucSettings, regionalAssignments, regionalSubstitutions };
}

export async function saveData(data) {
  await syncTable("cases", data.cases ?? []);
  await syncTable("employees", data.employees ?? []);
  await syncTable("queues", data.queues ?? []);
  await syncTable("state", data.state ?? []);
  await syncTable("vacations", data.vacations ?? []);
  await syncTable("journal", data.journal ?? []);
  await syncTable("settings", data.settings ?? []);
  await syncTable("yucSettings", data.yucSettings ?? []);
  await syncTable("regionalAssignments", data.regionalAssignments ?? []);
  await syncTable("regionalSubstitutions", data.regionalSubstitutions ?? []);
}
